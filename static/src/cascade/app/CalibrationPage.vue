    ...
    methods: {
      ...
      loadParTable() {
        return new Promise((resolve, reject) => {
          ...
          sciris.rpc('get_y_factors', [this.projectID, this.activeParset, this.$toolName])
            .then(response => {
      ...
      saveParTable() {
        return new Promise((resolve, reject) => {
          sciris.rpc('set_y_factors', [this.projectID, this.activeParset, this.parlist, this.$toolName])
            .then(response => {
              this.loadParTable()
                .then(response2 => {
      ...
      manualCalibration(project_id) {
        ...
        sciris.rpc('manual_calibration', [project_id, this.serverDatastoreId], {'parsetname':this.activeParset, 'plot_options':this.plotOptions,
          'plotyear':this.endYear, 'pops':this.activePop, 'tool': this.$toolName, 'cascade':null}) // Go to the server to get the results
          .then(response => {
      ...
      autoCalibrate(project_id) {
        ...
        sciris.rpc('automatic_calibration', [project_id, this.serverDatastoreId], {'parsetname':this.activeParset, 'max_time':maxtime, 'plot_options':this.plotOptions,
          'plotyear':this.endYear, 'pops':this.activePop, 'tool': this.$toolName, 'cascade':null}
